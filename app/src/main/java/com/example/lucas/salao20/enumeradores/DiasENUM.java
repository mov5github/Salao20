package com.example.lucas.salao20.enumeradores;

/**
 * Created by Lucas on 23/03/2017.
 */

public class DiasENUM {
    public static final String SEGUNDA = "segunda";
    public static final String TERCA = "terça";
    public static final String QUARTA = "quarta";
    public static final String QUINTA = "quinta";
    public static final String SEXTA = "sexta";
    public static final String SABADO = "sábado";
    public static final String DOMINGO = "domingo";
    public static final String ABRE = "abre";
    public static final String FECHA = "fecha";
    public static final String INICIO_ALMOCO = "inicioAlmoço";
    public static final String DURACAO_ALMOCO = "duraçãoAlmoço";
}
